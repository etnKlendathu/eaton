...
void s_nut_output_to_messages(std::vector<NutOutput>& assets, const fty::nut::DeviceConfigurations& output,
    discovered_devices_t* devices)
{
...
bool s_nut_dumpdata_to_fty_message(std::vector<fty_proto_t*>& assets,
    const fty::nut::DeviceConfiguration& dump, const fty::nut::KeyValues* mappings, const std::string& ip,
    const std::string& type)
{
    ...
    for (int i = startDevice; i <= endDevice; i++) {
        ...
        if (!fty_proto_ext_string(fmsg, "manufacturer", nullptr) ||
            !fty_proto_ext_string(fmsg, "model", nullptr)) {
...
void dump_data_actor(zsock_t* pipe, void* args)
{
    ...
    if (!valid) {
        ...
        fty::nut::DeviceConfiguration nutdata =
            fty::nut::dumpDevice(cpsr->nutDriver, addr, loop_nb, loop_iter_time, cpsr->documents);
        ...
        if (!nutdata.empty()) {
            ...
            if (s_valid_dumpdata(nutdata) &&
                ...
                log_debug(
                    "Dump data for %s (%s) failed: invalid data.", addr.c_str(), cpsr->nutDriver.c_str());
            ...
            log_debug(
                "Dump data for %s (%s) failed: failed to dump data.", addr.c_str(), cpsr->nutDriver.c_str());
...
void ScanNut::run(const CIDRList& list, const std::map<std::string, std::string>& devices,
    const fty::nut::KeyValues& nutMapping, const pack::StringList& docs)
{
    ...
    try {
        ...
        for (const secw::DocumentPtr& doc : secCreds) {
            ...
            auto credV3 = secw::Snmpv3::tryToCast(doc);
            auto credV1 = secw::Snmpv1::tryToCast(doc);
            if (credV3) {
    ...
    for (const secw::DocumentPtr& credential : credentialsV3) {
        ...
        result.deviceConfigurations = fty::nut::scanRangeDevices(fty::nut::SCAN_PROTOCOL_SNMP,
            ...
            list.lastAddress().toString(CIDR_WITHOUT_PREFIX), uint(timeout), result.documents);
    ...
    // NetXML scan.
    {
        ...
        result.deviceConfigurations = fty::nut::scanRangeDevices(fty::nut::SCAN_PROTOCOL_NETXML,
            list.firstAddress().toString(CIDR_WITHOUT_PREFIX),
            list.lastAddress().toString(CIDR_WITHOUT_PREFIX), uint(timeout));
